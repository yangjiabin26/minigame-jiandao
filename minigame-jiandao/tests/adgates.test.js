...
const adsThrow = { showRewarded: () => Promise.reject(new Error('boom')) };
function newState() {
  const mem = new Map();
...
test('广告 Promise 拒绝：三个入口均按失败降级，不抛异常，不卡流程', async () => {
  assert.strictEqual(await tryRevive(adsThrow, false), 'failed');
  assert.strictEqual(await tryDouble(adsThrow, 50), 50); // 原样返回，不翻倍
  const gs = newState();
  assert.strictEqual(await tryDiscount(adsThrow, gs, '2026-07-03'), false);
  assert.strictEqual(gs.discountLeft('2026-07-03'), 3); // 失败不消耗每日次数
});
